...
# give a random seed list example
random_seed_list = [28,37,10,900,27,78,6,235,12,89]
...
def main():
    ...
    for i in range(10):
        ...
        seed = random_seed_list[i]
        print(seed)
        random.seed(seed)
        np.random.seed(seed)
        torch.manual_seed(seed)
        # Create the appnp model
        model = APPNP(in_feats=24, hiddens=[128, 128], n_classes=2, activation=F.relu, feat_drop=0.1, edge_drop=0, alpha=0.5, k=3)
        ...
        with torch.no_grad():
            ...
            with open("appnp_with_results.txt", "a") as f:
